...
def main():
    ...
    curr_dt = start_dt 
    days_events = c.get_events_for_day(curr_dt) # string of all events 
#    print(days_events)
    orig_stdout, orig_stderr = sys.stdout, sys.stderr
    sys.stdout, sys.stderr = None, None

    c = process_cal4.process_cal(args.file) #creates a process_cal instance with attribute filename

    newline = ""

    curr_dt = start_dt
    days_events = c.get_events_for_day(curr_dt) #creates 
    if (days_events):
